...
// Lit material use specular setup instead of metallic setup.
...
// position : Intersection between Ray and Scene.
// distance : Distance from Ray's starting position to intersection.
// normal   : Normal direction of the intersection.
...
// Modified from HDRP's ScreenSpaceReflection.compute. GGX VNDF via importance sampling.
...
void HitSurfaceDataFromGBuffers(float2 screenUV, inout half3 albedo, inout half3 specular, inout half occlusion, inout half3 normal, inout half smoothness)
{
    ...
    albedo = gBuffer0.rgb;
    // Specular & Metallic setup conversion.
    specular = (UnpackMaterialFlags(gBuffer0.a) == kMaterialFlagSpecularSetup) ? gBuffer1.rgb : lerp(kDieletricSpec.rgb, max(albedo.rgb, kDieletricSpec.rgb), gBuffer1.r);
...
float GenerateRandomFloat(float2 screenUV)
{
    ...
    // Accumulate noise over time (frames).
...
RayHit RayMarching(Ray ray, half dither, float distance)
{
    ...
    for (uint i = 1; i <= MAX_STEP; i++)
    {
        ...
        // Non-GL (DirectX): rayPositionNDC.z is (near to far) 1..0
        float hitDepth = ConvertLinearEyeDepth(rayPositionNDC.z); 
        float depthDiff = sceneDepth - hitDepth;

#if defined(_BACKFACE_ENABLED)
        float deviceBackDepth = SAMPLE_TEXTURE2D_X_LOD(_CameraBackDepthTexture, sampler_PointClamp, rayPositionNDC.xy, 0).r;
        // Avoid infinite thickness for objects with no thickness (ex. Plane).
    #if (UNITY_REVERSED_Z == 1)
        bool backDepthValid = deviceBackDepth != 0.0 ? true : false;
    #else
        bool backDepthValid = deviceBackDepth != 1.0 ? true : false;
    #endif
    ...
    #else
        isSky = deviceDepth == 1.0 ? true : false;
    #endif
