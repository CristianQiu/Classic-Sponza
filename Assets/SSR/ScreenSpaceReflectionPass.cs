...
public class ScreenSpaceReflectionPass : ScriptableRenderPass
{
	...
	private class PassData
	{
		...
		public TextureHandle ssrHitHandle;
		public TextureHandle reflectHandle;
		public TextureHandle historyHandle;
	}
	...
	public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
	{
		...
		//using (IUnsafeRenderGraphBuilder builder = renderGraph.AddUnsafePass("Screen Space Hit Pass", out PassData passData, profilingSampler))
		//{
		//	builder.AllowPassCulling(false);

		//	passData.stage = PassStage.One;
		//	// TODO: humm is this camera depth or camera depth texture?
		//	passData.source = resourceData.cameraDepth;
		//	passData.target = textureHandles.sourceHandle;
		//	passData.material = ssrMaterial;
		//	passData.materialPassIndex = 0;

		//	builder.UseTexture(resourceData.cameraDepth);
		//	builder.UseTexture(resourceData.cameraDepthTexture);
		//	builder.UseTexture(textureHandles.sourceHandle);
		...
		using (IRasterRenderGraphBuilder builder = renderGraph.AddRasterRenderPass("Screen Space Hit Pass", out PassData passData, profilingSampler))
		{
			...
			builder.SetRenderAttachment(textureHandles.sourceHandle, 0, AccessFlags.WriteAll);
			builder.UseTexture(resourceData.cameraDepth);
			builder.UseTexture(resourceData.cameraDepthTexture);
		...
		using (IRasterRenderGraphBuilder builder = renderGraph.AddRasterRenderPass("Resolve Color Pass", out PassData passData, profilingSampler))
		{
			...
			passData.ssrHitHandle = textureHandles.sourceHandle; // ssr hit texture

			builder.SetRenderAttachment(textureHandles.reflectHandle, 0, AccessFlags.WriteAll);
	...
	private static void ExecutePass(PassData passData, RasterGraphContext context)
	{
		...
		{
			passData.material.SetTexture("_ScreenSpaceReflectionHitTexture", passData.ssrHitHandle);
		}
		else
		{
		}

		Blitter.BlitTexture(context.cmd, passData.source, Vector3.one, passData.material, passData.materialPassIndex);
	}
	...
	private TextureHandles CreateRenderGraphTextures(RenderGraph renderGraph, UniversalRenderingData renderingData, UniversalCameraData cameraData, UniversalResourceData resourceData)
	{
		...
		//cameraDesc.clearBuffer = true;
		//cameraDesc.clearColor = Color.clear;
		...
		textureHandles.sourceHandle = renderGraph.CreateTexture(hitDesc);
	...
	private static void UpdateMaterialProperties(Material ssrMaterial)
	{
		...
		//ssrMaterial.SetTexture(Shader.PropertyToID("_ScreenSpaceReflectionHistoryTexture"), history);
